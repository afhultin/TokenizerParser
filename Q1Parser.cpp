...
// Tokens names
enum Tokens {
    LETTER, DIGIT, INT_LIT = 10, FLOAT_LIT, IDENT, ASSIGN_OP = 20, ADD_OP,
    SUB_OP, EQUAL, MULT_OP, DIV_OP, LEFT_PAREN, RIGHT_PAREN, PERIOD,
    INT_KEYWORD = 40, FLOAT_KEYWORD, COMMA, DECL, ENDFILE = 80, UNKNOWN = 99
};
...
/************************************************/
/* assign - parse assignment statement by the rule: <assign> -> <identifier> = <expr> */
...
/*************************************************/
/* <floatVar> - parse float variable by the rule: <floatVar> -> <ident> | <ident><floatLit> */
Tokens floatVar() {
    ...
    if (nextToken == ASSIGN_OP) {
        ...
        if (nextToken == FLOAT_LIT) {
            float value = atof(lexeme.c_str());  // convert string to float
            addSymbolTable(varName, "float", 0, value);  // add identifier to the symbol table
            nextToken = tokenizer();
        } else
            errMsg("Float expected, found " + lexeme);
    } else {
...
/*************************************************/
/* <intVar> - parse integer variable by the rule: <intVar> -> <ident> | <ident><intLit> */
Tokens intVar() {
    string varName = lexeme;
    Tokens nextToken = tokenizer();
    if (nextToken == ASSIGN_OP) {
        ...
        if (nextToken == INT_LIT) {
            ...
            errMsg("Integer expected, found " + lexeme);
    } else {
...
/*************************************************/
/* <floatIdent> - float identifier parser by the rule: <floatIdent> -> <floatVar> <floatVar>* */
Tokens floatIdent() {
    Tokens nextToken;
    if (nextToken == IDENT) {
...
/*************************************************/
/* <intIdent> - integer identifier parser by the rule: <intIdent> -> <intVar> <intVar>* */
Tokens intIdent() {
    Tokens nextToken;
    if (nextToken == IDENT) {
        while (nextToken == IDENT) {
...
/***************************************************/
/* decl - parse declaration statement by the rule <ident> -> int <intIdent> | float <floatIdent> */
        ...
        while (nextToken == IDENT) {
            ...
            if (nextToken == EQUAL) {
                ...
                if (nextToken == INT_LIT && type == "int") {
                    ...
                    errMsg("int or float literal expected, found " + lexeme);
                nextToken = tokenizer();
            } else
                addSymbolTable(name, type, 0, 0);
            if (nextToken == COMMA) nextToken = tokenizer();
            else break;
        }
    else
...
int main() {
    ...
    if (!infp) {
        ...
        nextChar = ' '; // Initialize nextChar to the first character from the file
        Tokens nextToken = tokenizer();
        do 
        {
